...
import { PHALA_APP_COMPOSE_WIRE_HASH_SEMANTICS } from "./phala-app-compose-wire-core.mjs";
...
export const CVM_DESCRIPTOR_RUNTIME_AUTHORITY_DOMAIN =
  "dnai-wikigen/cvm-descriptor-runtime-authority/v3\0";
export const CVM_DESCRIPTOR_RUNTIME_FACTS_DOMAIN =
  "dnai-wikigen/cvm-descriptor-runtime-facts/v2\0";
export const CVM_DESCRIPTOR_DOMAIN_RUNTIME_FACTS_DOMAIN =
  "dnai-wikigen/cvm-descriptor-domain-runtime-facts/v2\0";
...
export const CVM_DESCRIPTOR_RUNTIME_FACT_SOURCES = Object.freeze({
  ...
  app_compose_hash:
    "strict_expected_post_sdk_transform_compose_hash_pending_authenticated_staging_server_equality",
  pre_transform_app_compose_hash:
    "exact_reviewed_candidate_audit_hash_not_runtime_identity",
...
function same(value, expected) {
  return JSON.stringify(sortedObject(value)) === JSON.stringify(sortedObject(expected));
}
...
export function normalizeCvmDescriptorDomainRuntimeFacts(value, domain) {
  ...
  if (parsed.domain !== domain
    ...
    || typeof parsed.pre_transform_app_compose_hash !== "string"
    || !BARE_SHA256.test(parsed.pre_transform_app_compose_hash)
    || parsed.pre_transform_app_compose_hash === parsed.app_compose_hash) {
  ...
  const normalized = {
    ...
    pre_transform_app_compose_hash: parsed.pre_transform_app_compose_hash,
...
export function createCvmDescriptorDomainRuntimeFacts({
  ...
  const facts = {
    ...
    pre_transform_app_compose_hash: preTransformAppComposeHash,
    service_images: serviceImages,
...
function runtimeFactsDigestInput(value) {
  return {
    ...
    pre_transform_app_compose_hash_by_domain: value.pre_transform_app_compose_hash_by_domain,
    compose_hash_semantics: value.compose_hash_semantics,
...
export function normalizeCvmDescriptorRuntimeAuthority(value) {
  ...
  const parsed = exactRecord(value, [
    ...
    "pre_transform_app_compose_hash_by_domain",
    "compose_hash_semantics",
  ...
  if (parsed.schema !== CVM_DESCRIPTOR_RUNTIME_AUTHORITY_SCHEMA
    ...
    || !same(parsed.compose_hash_semantics, PHALA_APP_COMPOSE_WIRE_HASH_SEMANTICS)
  ...
  const preComposeMap = exactRecord(parsed.pre_transform_app_compose_hash_by_domain,
    CVM_LAUNCH_DOMAINS, "pre-transform AppCompose audit hash map");
  const preTransformAppComposeHashByDomain = {};
  for (const [index, domain] of CVM_LAUNCH_DOMAINS.entries()) {
    ...
    if (descriptorSha256ByDomain[domain] !== descriptors[index].descriptor_sha256
      ...
      || preComposeMap[domain] !== descriptors[index].pre_transform_app_compose_hash) {
      throw new Error(`${domain} aggregate maps do not equal exact runtime facts`);
    }
    appComposeHashByDomain[domain] = composeMap[domain];
    preTransformAppComposeHashByDomain[domain] = preComposeMap[domain];
  }
  ...
  const normalized = {
    ...
    tinker_account_binding_ceremony_receipt_sha256: exactDigest(
    ...
    pre_transform_app_compose_hash_by_domain: preTransformAppComposeHashByDomain,
    compose_hash_semantics: structuredClone(PHALA_APP_COMPOSE_WIRE_HASH_SEMANTICS),
