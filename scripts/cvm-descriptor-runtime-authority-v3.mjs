...
function projectDomainFacts(domain, file) {
  ...
  return createCvmDescriptorDomainRuntimeFacts({
    ...
    appComposeHash: phalaAppComposeExpectedRuntimeHash(appCompose),
    preTransformAppComposeHash: phalaDstackComposeHash(appCompose),
    serviceImages,
...
export async function createFreshCvmDescriptorRuntimeAuthority(input = {}) {
  ...
  try {
    ...
    const authority = {
      ...
      pre_transform_app_compose_hash_by_domain: Object.fromEntries(descriptors.map((entry) => [
        entry.domain,
        entry.pre_transform_app_compose_hash,
      ])),
  ...
  try {
    ...
    for (const domain of CVM_LAUNCH_DOMAINS) {
      ...
      if (file.sha256 !== facts.descriptor_sha256
        ...
        || phalaAppComposeExpectedRuntimeHash(appCompose) !== facts.app_compose_hash
        || phalaDstackComposeHash(appCompose) !== facts.pre_transform_app_compose_hash) {
      ...
      materials.push({
        ...
        pre_transform_app_compose_hash: facts.pre_transform_app_compose_hash,
      });
    }
    const after = await directory.handle.stat({ bigint: true });
    const namedAfter = await lstat(directory.resolved, { bigint: true });
