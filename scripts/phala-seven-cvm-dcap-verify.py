...
descriptors into a root-owned system-Python bootstrap.  The bootstrap loads
only the immutable native extension from fd 3 and executes these exact source bytes from fd 4.  No uv,
...
PINNED_MACOS_VERSION = "26.5.1"
PINNED_MACOS_BUILD = "25F80"
...
def _assert_safe_opened_runtime() -> None:
    ...
    if (
        ...
        or getattr(dcap_qvl, "__native_path__", None) != "/dev/fd/3"
        or native_spec.origin != dcap_qvl.__native_path__
        or getattr(native, "__file__", None) != dcap_qvl.__native_path__
        or getattr(dcap_qvl, "__native_authority_mode__", None)
        not in ("root_owned_production", "operator_owned_node_test")
        ...
        raise ValueError
    native_authority = os.fstat(3)
    if (
        not stat.S_ISREG(native_authority.st_mode)
        or native_authority.st_nlink != 1
        or stat.S_IMODE(native_authority.st_mode) & 0o022
        or (
            dcap_qvl.__native_authority_mode__ == "root_owned_production"
            and (
                native_authority.st_uid != 0
                or stat.S_IMODE(native_authority.st_mode) != 0o555
            )
        )
        or (
            dcap_qvl.__native_authority_mode__ == "operator_owned_node_test"
            and native_authority.st_uid != os.geteuid()
        )
    ):
        raise ValueError
    for digest in (
    ...
    for module in tuple(sys.modules.values()):
        ...
        if not isinstance(location, str) or location in ("/dev/fd/3", "/dev/fd/4"):
            continue
...
async def _verify(
    ...
    verified = await dcap_qvl.get_collateral_and_verify(raw, PCCS_URL)
...
def main() -> int:
    try:
        ...
        parsed = _exact_object(
            payload,
            ("quote", "measurement_policy", "measurement_policy_sha256"),
        )
        quote = parsed["quote"]
        ...
        result = asyncio.run(_verify(raw, policy, policy_sha256))
        _assert_safe_opened_runtime()
        result["runtime_environment_sha256"] = runtime_sha256
