...
function normalizeTopology(text, files, descriptors, manifest, expectedReleaseSha) {
  ...
  const topology = parseJson(text, "CVM topology");
...
export async function validateCanonicalGeneratedCvmDescriptorSet({
  ...
  const [manifestFile, bundleFile, deploymentIntentFile] = await Promise.all([
...
export async function main(argv = process.argv.slice(2), io = console) {
  try {
    ...
    if (args.command === "help") {
      io.log("Usage: node scripts/cvm-release-descriptor-set.mjs check --repo-root DIR --release-dir DIR/.release --release-sha 40_HEX");
      return 0;
    }
