...
contract EmailOracleAuthScript is Script {
    function run() public {
        require(
            block.chainid != 84532 && block.chainid != 8453, "use DeployFreshSuite for Base EmailOracleAuth releases"
        );
        ...
        require(
            !freezeOracle && !freezeConsumers, "local helper cannot create a production-frozen EmailOracleAuth release"
        );
        ...
        EmailOracleAuth auth =
            new EmailOracleAuth(owner, upgradeDelay, allowAnyDevice, initialDeviceId, initialComposeHash, false);
