...
if [ "$DNAI_KEYSTORE_HELPER_CANONICAL_PATH" != "$DNAI_KEYSTORE_ENVIRONMENT_HELPER" ] || [ ! -f "$DNAI_KEYSTORE_ENVIRONMENT_HELPER" ] || [ -L "$DNAI_KEYSTORE_ENVIRONMENT_HELPER" ] || [ ! -r "$DNAI_KEYSTORE_ENVIRONMENT_HELPER" ]; then
...
if [ "$DNAI_KEYSTORE_HELPER_OWNER" != "$EUID" ] || [ "$DNAI_KEYSTORE_HELPER_LINKS" != "1" ] || (( (8#$DNAI_KEYSTORE_HELPER_MODE & 0022) != 0 )); then
...
if [[ ! "$DNAI_KEYSTORE_HELPER_PARENT_METADATA" =~ ^[0-9]+:[0-7]+$ ]]; then DNAI_KEYSTORE_HELPER_PARENT_METADATA="$(/usr/bin/stat -c '%u:%a' -- "$DNAI_KEYSTORE_HELPER_PARENT" 2>/dev/null || true)"; fi
